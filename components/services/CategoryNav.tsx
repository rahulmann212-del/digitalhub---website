...
export default function CategoryNav() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Agar user top par hai (200px ke andar), toh hamesha dikhao
      if (currentScrollY < 200) {
        setIsVisible(true);
      } else {
        // Agar user niche scroll kar raha hai toh hide karo, upar aa raha hai toh show karo
  ...
  return (
    <div 
      className={`sticky top-16 md:top-20 z-40 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 shadow-sm transition-all duration-500 ease-in-out ${
        isVisible 
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => handleScrollClick(cat.id)}
